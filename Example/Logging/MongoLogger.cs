...
//        public async Task<List<LogRecord>> List()
//        {
//            return await base.GetAll( this.TableName );
//        }
...
//            // uses a literal DB field which needs to be changed
//            List<LogRecord> results = await collection.Find( FilterDefinition<LogRecord>.Empty ).Sort( "{timestamp: -1}" ).Limit( top ).ToListAsync();

//            return ( results != null && results.Count > 0 ) ? results : new List<LogRecord>();
//        }

//        /// <summary>Get logs only on a single day</summary>
//        public async Task<List<LogRecord>> ListByDay( DateTimeOffset day )
//        {
//            return await this.ListByRange( day.StartOfDay(), day.EndOfDay() );
//        }
...
//            IMongoCollection<LogRecord> collection = this.Database.GetCollection<LogRecord>( this.TableName );
//            return await collection.Find<LogRecord>( t => t.Timestamp >= begin && t.Timestamp < end ).ToListAsync();
//        }
...
//                _ = base.Insert( new LogRecord( msg, lvl ) { Category = category, Source = logger, Args = args },
//                        this.TableName );
//            }
